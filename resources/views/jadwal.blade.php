...
<body>
    ...
    <section class="container mx-auto py-11 px-4">
        ...
        <div class="flex flex-col items-center mb-8">
            <div class="bg-white p-4 rounded-xl shadow-lg border border-pink-200 hover:shadow-2xl transition-shadow duration-300 w-full max-w-5xl">
                <form method="GET" action="{{ route('jadwal.form') }}" class="text-center mb-6">
                    <label for="tanggal" class="block text-lg font-semibold text-gray-700 mb-2">Pilih Tanggal:</label>
                    <input type="date" name="tanggal" id="tanggal" value="{{ $tanggal }}"
                        class="px-4 py-2 md:px-6 md:py-3 border-2 border-pink-300 rounded-lg 
                            text-sm md:text-lg font-medium text-gray-700
                            focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500
                            hover:border-pink-400 transition-colors duration-300
                            cursor-pointer">
                    <button type="submit" class="ml-2 px-4 py-2 bg-pink-500 text-white rounded hover:bg-pink-600 transition-all">
                        Tampilkan
                    </button>
                </form>

                <div class="overflow-x-auto bg-white p-4 md:p-6 rounded-xl shadow-md">
                    <table class="w-full min-w-max border-collapse">
                        <thead>
                            <tr>
                                <th class="p-2 border text-xs md:text-sm">Ruangan</th>
                                @for ($i = 7; $i <= 22; $i++)
                                    <th class="p-2 border text-xs md:text-sm whitespace-nowrap">{{ str_pad($i, 2, '0', STR_PAD_LEFT) }}:00</th>
                                    @endfor
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($jadwal as $ps)
                            <tr>
                                <td class="p-2 border font-semibold bg-gray-100 text-xs md:text-sm whitespace-nowrap">
                                    {{ $ps['name'] }} ({{ $ps['type'] }})
                                </td>
                                @for ($i = 7; $i <= 22; $i++)
                                    @php
                                    $status=$ps['slots'][$i] ?? null;
                                    @endphp
                                    <td class="p-2 border text-xs md:text-sm text-center {{ $status ? 'bg-red-200 text-red-800 font-bold' : 'bg-green-100 text-green-800' }}">
                                    {{ $status ?? 'Kosong' }}
                                    </td>
                                    @endfor
                            </tr>
                ...
                <div class="mt-6 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded text-sm md:text-base">
